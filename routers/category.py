...
def create_category(request: schemas.Category, db : Session = Depends(get_db), get_current_user: schemas.Login = Depends(Oauth2.get_current_user)):
    return rcategory.create(request, db)

@router.put('/update/{id}', status_code = 200)
def update_category(id: int, request : schemas.UpdateCategory, db : Session = Depends(get_db),  get_current_user: schemas.Login = Depends(Oauth2.get_current_user)):
    return rcategory.update(id, request, db)

@router.delete('/delete/{id}', status_code = 200)
def delete_category(id: int, db : Session = Depends(get_db), get_current_user: schemas.Login = Depends(Oauth2.get_current_user)): 
